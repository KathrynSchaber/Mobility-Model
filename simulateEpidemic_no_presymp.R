simulateEpidemic_no_presymp = function(hh, T = 200)
{
  ...
  B_rest_mats[[5]] <- B_rest_norm
  ## time point "41" is the normal B values before epidemic (keep at 41 so days of infectiousness 1-40 can have matrices 1-40)
  B_home_norm = diag(as.vector(M), Nf, Nf) %*% t(home_U)
  B_home_mats[[5]] <- B_home_norm
  ...
  mosq_house_mats[[5]] <- mosq_house_norm
  ####
  
  
  
  ## create R_nonlinear_norm matrix
  #R_nonlinear_norm = 1 - exp(-b * c * t(B) %*% Q %*% t(U) * rho_mean)
  #### Original versino had "c*rho_mean" for set value c and duration of infectiousness
  #### This version has the c_1 values for each time period of infectiousness (5)
  #### rho_mean is 4.4, but the 5th time point has c value of 0.01, so shouldn't be a big issue
  R_nonlinear_norm = 1 - exp(-b * (0.7+0.4+0.1+0.01) * t(B_norm) %*% Q %*% t(U))
  diag(R_nonlinear_norm) = 0
  R_nonlinear_norm_rest = 1 - exp(-b * (0.7+0.4+0.1+0.01) * t(B_rest_norm) %*% Q %*% t(U))
  diag(R_nonlinear_norm_rest) = 0
  R_nonlinear_norm_home = 1 - exp(-b * (0.7+0.4+0.1+0.01) * t(B_home_norm) %*% Q %*% t(U))
  diag(R_nonlinear_norm_home) = 0
  
  #############
  V_norm_secondary_home <- t(B_norm) %*% Q %*% t(U)
  V_norm_secondary_rest <- t(B_norm) %*% Q %*% t(U)
  for(ii in 1:Nh){
    V_norm_secondary_home[ii,which(name_homes != name_homes[ii])] <- 0
    V_norm_secondary_rest[ii,which(name_homes == name_homes[ii])] <- 0
  }
  
  R_nonlinear_norm_secondary_home = 1 - exp(-b * (0.7+0.4+0.1+0.01) * V_norm_secondary_home)
  diag(R_nonlinear_norm_secondary_home) = 0
  R_nonlinear_norm_secondary_rest = 1 - exp(-b * (0.7+0.4+0.1+0.01) * V_norm_secondary_rest)
  ...
  for(tt in 2 : (T + 1)){
    ...
    c_1[which(Ih[,1] == 1)] = 0.7
    c_1[which(Ih[,2] == 1)] = 0.4
    c_1[which(Ih[,3] == 1)] = 0.1
    c_1[which(Ih[,4] == 1)] = 0.01
    ...
    for(ii in 1:Nh){
      if(ii %in% home_body_old){}
        ...
        if(Ih[ii,1] == 1){ # Ih[ii,1] is presymptomatic period
    ...
    for(ii in 1:rho_max){
      ind_temp <- which(Ih[,ii] == 1)
      if(length(ind_temp) > 0){
        V_move_secondary_home[[ii]][ind_temp,] <- (t(B_temp) %*% Q %*% t(U))[ind_temp,]
        V_move_secondary_home[[ii]][ind_temp, which(name_homes != name_homes[ind_temp])] <- 0
      }
    }
    ...
    for(ii in 1:rho_max){
      ind_temp <- which(Ih[,ii] == 1)
      if(length(ind_temp) > 0){
        V_move_secondary_rest[[ii]][ind_temp,] <- (t(B_temp) %*% Q %*% t(U))[ind_temp,]
        V_move_secondary_rest[[ii]][ind_temp, which(name_homes == name_homes[ind_temp])] <- 0
      }
    }
    ######
  }
  ...
  for(ii in 1:Nh){
    if(sum(is.na(V_move_rest[[4]][ii,])) < Nh){
      rate_temp <- ((0.7 * V_move_rest[[1]][ii,]) + (0.4 * V_move_rest[[2]][ii,]) + (0.1 * V_move_rest[[3]][ii,]) +
                      (0.01 * V_move_rest[[4]][ii,]))
      ...
      rate_temp <- ((0.7 * V_move_rest[[1]][ii,]) + (0.4 * V_move_rest[[2]][ii,]))
      R_nonlinear_movement_rest[ii,] = 1 - exp(-b * rate_temp)
    }
    else if(sum(is.na(V_move_rest[[1]][ii,])) < Nh){
      rate_temp <- ((0.7 * V_move_rest[[1]][ii,]))
      R_nonlinear_movement_rest[ii,] = 1 - exp(-b * rate_temp)
    }
  }
  ...
  for(ii in 1:Nh){
    if(sum(is.na(V_move_home[[4]][ii,])) < Nh){
      rate_temp <- ((0.7 * V_move_home[[1]][ii,]) + (0.4 * V_move_home[[2]][ii,]) + (0.1 * V_move_home[[3]][ii,]) +
                      (0.01 * V_move_home[[4]][ii,]))
      ...
      rate_temp <- ((0.7 * V_move_home[[1]][ii,]) + (0.4 * V_move_home[[2]][ii,]))
      R_nonlinear_movement_home[ii,] = 1 - exp(-b * rate_temp)
    }
    else if(sum(is.na(V_move_home[[1]][ii,])) < Nh){
      rate_temp <- ((0.7 * V_move_home[[1]][ii,]))
      R_nonlinear_movement_home[ii,] = 1 - exp(-b * rate_temp)
    }
  }
  ...
  ## set up R_nonlinear_movement
  for(ii in 1:Nh){
    if(sum(is.na(V_k_l_vecs[[4]][ii,])) < Nh){
      rate_temp <- ((0.7 * V_k_l_vecs[[1]][ii,]) + (0.4 * V_k_l_vecs[[2]][ii,]) +
                      (0.1 * V_k_l_vecs[[3]][ii,]) + (0.01 * V_k_l_vecs[[4]][ii,]))
      ...
      rate_temp <- ((0.7 * V_k_l_vecs[[1]][ii,]))
      R_nonlinear_movement[ii,] = 1 - exp(-b * rate_temp)
    }
  }
  #######################
  for(ii in 1:Nh){
    if(sum(is.na(V_move_secondary_home[[4]][ii,])) < Nh){
      rate_temp <- ((0.7 * V_move_secondary_home[[1]][ii,]) + (0.4 * V_move_secondary_home[[2]][ii,]) + (0.1 * V_move_secondary_home[[3]][ii,]) +
                      (0.01 * V_move_secondary_home[[4]][ii,]))
      ...
      rate_temp <- ((0.7 * V_move_secondary_home[[1]][ii,]) + (0.4 * V_move_secondary_home[[2]][ii,]))
      R_nonlinear_movement_secondary_home[ii,] = 1 - exp(-b * rate_temp)
    }
    else if(sum(is.na(V_move_secondary_home[[1]][ii,])) < Nh){
      rate_temp <- ((0.7 * V_move_secondary_home[[1]][ii,]))
      R_nonlinear_movement_secondary_home[ii,] = 1 - exp(-b * rate_temp)
    }
  }
  ...
  for(ii in 1:Nh){
    if(sum(is.na(V_move_secondary_rest[[4]][ii,])) < Nh){
      rate_temp <- ((0.7 * V_move_secondary_rest[[1]][ii,]) + (0.4 * V_move_secondary_rest[[2]][ii,]) + (0.1 * V_move_secondary_rest[[3]][ii,]) +
                      (0.01 * V_move_secondary_rest[[4]][ii,]))
      ...
      rate_temp <- ((0.7 * V_move_secondary_rest[[1]][ii,]) + (0.4 * V_move_secondary_rest[[2]][ii,]))
      R_nonlinear_movement_secondary_rest[ii,] = 1 - exp(-b * rate_temp)
    }
    else if(sum(is.na(V_move_secondary_rest[[1]][ii,])) < Nh){
      rate_temp <- ((0.7 * V_move_secondary_rest[[1]][ii,]))
      R_nonlinear_movement_secondary_rest[ii,] = 1 - exp(-b * rate_temp)
    }
  }
  ...
  return(list(EIR_cum, Ih_sum, B_mats, SN_out_deg, 
              H_deg, R_nonlinear_norm, R_nonlinear_movement, U_omega,
              mosq_house_mats, name_homes, H_deg_houses, 
              R_nonlinear_norm_rest, R_nonlinear_norm_home,
              R_nonlinear_movement_home, R_nonlinear_movement_rest,
