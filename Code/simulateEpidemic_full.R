simulateEpidemic_full = function(hh, T = 200)
{   
  ...
  R_nonlinear_norm = 1 - exp(-b * (0.4+0.7+0.4+0.1+0.01) * t(B_norm) %*% Q %*% t(U))
  diag(R_nonlinear_norm) = 0
  
  R_nonlinear_norm_rest = 1 - exp(-b * (0.4+0.7+0.4+0.1+0.01) * t(B_rest_norm) %*% Q %*% t(U))
  diag(R_nonlinear_norm_rest) = 0
  R_nonlinear_norm_home = 1 - exp(-b * (0.4+0.7+0.4+0.1+0.01) * t(B_home_norm) %*% Q %*% t(U))
  ...
  R_nonlinear_norm_secondary_home = 1 - exp(-b * (0.4+0.7+0.4+0.1+0.01) * V_norm_secondary_home)
  diag(R_nonlinear_norm_secondary_home) = 0
  R_nonlinear_norm_secondary_rest = 1 - exp(-b * (0.4+0.7+0.4+0.1+0.01) * V_norm_secondary_rest)
  ...
  for(tt in 2 : (T + 1)){
    ...
    c_1[which(Ih[,1] == 1)] = 0.4 ## presymptomatic period
    c_1[which(Ih[,2] == 1)] = 0.7
    c_1[which(Ih[,3] == 1)] = 0.4
    c_1[which(Ih[,4] == 1)] = 0.1
    c_1[which(Ih[,5] == 1)] = 0.01
    ...
    for(ii in 1:Nh){
      if(ii %in% home_body_old){}
        ...
        if(Ih[ii,2] == 1){ # Ih[ii,1] is presymptomatic period
    ...
    for(ii in 1:rho_max){
      ...
      if(length(ind_temp) > 0){
        ...
        for(x in 1:(length(ind_temp))){
          V_move_secondary_home[[ii]][ind_temp[x], which(name_homes != name_homes[ind_temp[x]])] <- 0
        }
      }
    }
    ...
    for(ii in 1:rho_max){
      ...
      if(length(ind_temp) > 0){
        ...
        for(x in 1:(length(ind_temp))){
          V_move_secondary_rest[[ii]][ind_temp[x], which(name_homes == name_homes[ind_temp[x]])] <- 0
        }
      }
    }
    ######
